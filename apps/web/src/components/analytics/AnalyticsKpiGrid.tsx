          ...
          <Card className="h-full transition-colors hover:bg-muted/30">
            <CardContent className="p-4">
              ...
              <div className="mt-2 flex items-center gap-1 text-xs text-muted-foreground">
                {card.delta == null ? null : card.delta >= 0 ? <ArrowUp className="h-3 w-3" /> : <ArrowDown className="h-3 w-3" />}
                {card.delta == null ? 'No previous period' : `${card.delta >= 0 ? '+' : ''}${card.delta.toFixed(1)}% vs previous`}
              </div>
...
function MiniSparkline({ values }: { values: number[] }) {
  if (values.length < 2) return <div className="mt-3 h-6 rounded bg-muted/40" />;
  ...
  return (
    <svg className="mt-3 h-6 w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      <polyline fill="none" stroke="currentColor" strokeWidth="4" points={points} className="text-primary/70" />
    </svg>
  );
}
