...
TorqueCore Supabase is not integrated yet. Use separately named `TORQUECORE_*` server-only variables so Radar's own Supabase configuration cannot be confused with the remote shipment data plane.
